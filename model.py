...
class AlexNet(nn.Module):
    def __init__(self, num_classes=360):
        ...
        conv1 = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=11, stride=4, padding=2), #nn.Conv2d(1, 64, kernel_size=11, stride=4, padding=3),
            nn.BatchNorm2d(64),
    ...
    def forward(self, x, out_feat_keys=None):
        """
        This function forwards an image `x` through the network and return the asked output features.
        x: input image.
        out_feat_keys: a list/tuple with the feature names of the features that the function should return.
        By default the last feature of the network is returned.
        Return: out_feats: If multiple output features were asked then `out_feats` is a list with the asked
                output features placed in the same order as in `out_feat_keys`.
                If a single output feature was asked then `out_feats` is that output feature (and not a list).
        """
